...
// ============================================
// INTERACTIVE REAL-TIME TASK MANAGEMENT SYSTEM
// With Advanced State Management & CRUD Operations
// ============================================
...
const INITIAL_DATA = {
  columns: {
    ...
    complete: { id: "complete", title: "Complete", color: "#B2E4BA", dotColor: "#B2E4BA", tasks: ["t10", "t11"] },
  },
  tasks: {
    t1: { id: "t1", title: "Homepage UI Design Draft", desc: "Building the first version of homepage layout focusing on usability and flow.", tags: ["Design UI/UX", "Frontend"], progress: 0, total: 4, files: 3, comments: 1, assignees: ["MA", "SC"], createdAt: SEED_TIMESTAMP },
    t2: { id: "t2", title: "Product Detail Wireframe", desc: "Wireframing the product detail page with focus on images and description.", tags: ["UX", "Research"], progress: 0, total: 4, files: 2, comments: 3, assignees: ["DJ", "JW"], createdAt: SEED_TIMESTAMP },
    t3: { id: "t3", title: "Shopping Cart Structure", desc: "Planning the shopping cart page to ensure user-friendly checkout flow.", tags: ["Backend", "API"], progress: 0, total: 4, files: 3, comments: 5, assignees: ["MA", "SC"], createdAt: SEED_TIMESTAMP },
    t4: { id: "t4", title: "User Registration Flow", desc: "Building user account creation system with secure authentication.", tags: ["Frontend", "Auth"], progress: 1, total: 4, files: 3, comments: 8, assignees: ["DJ", "MA"], createdAt: SEED_TIMESTAMP },
    t5: { id: "t5", title: "Product Catalog Layout", desc: "Creating responsive product grid for better shopping experience.", tags: ["Design UI/UX", "Frontend"], progress: 2, total: 4, files: 3, comments: 11, assignees: ["SC", "JW"], createdAt: SEED_TIMESTAMP },
    t6: { id: "t6", title: "Payment Gateway Setup", desc: "Integrating payment gateway API to support multiple methods.", tags: ["Backend", "API"], progress: 3, total: 4, files: 2, comments: 9, assignees: ["MA", "DJ"], createdAt: SEED_TIMESTAMP },
    t7: { id: "t7", title: "Homepage Responsive Check", desc: "Testing homepage layout across devices to ensure responsiveness.", tags: ["QA", "Frontend"], progress: 2, total: 4, files: 2, comments: 18, assignees: ["SC", "MA"], createdAt: SEED_TIMESTAMP },
    t8: { id: "t8", title: "Product Image Optimization", desc: "Checking optimized images for clarity and performance balance.", tags: ["Media", "Performance"], progress: 3, total: 4, files: 3, comments: 16, assignees: ["JW", "SC"], createdAt: SEED_TIMESTAMP },
    t9: { id: "t9", title: "Checkout Flow Testing", desc: "Reviewing checkout process to ensure smooth user experience.", tags: ["QA", "Backend"], progress: 3, total: 4, files: 2, comments: 5, assignees: ["MA", "DJ"], createdAt: SEED_TIMESTAMP },
    t10: { id: "t10", title: "Login Page Interface", desc: "Finished building login interface including input validation.", tags: ["Design UI/UX", "Frontend"], progress: 4, total: 4, files: 1, comments: 8, assignees: ["MA", "SC"], createdAt: SEED_TIMESTAMP },
    t11: { id: "t11", title: "Database Schema Setup", desc: "Delivered database setup with tables for users, orders, and items.", tags: ["Backend", "Database"], progress: 4, total: 4, files: 3, comments: 19, assignees: ["DJ", "MA"], createdAt: SEED_TIMESTAMP },
  },
};
...
// Advanced reducer with undo/redo history
function appReducer(state, action) {
  switch (action.type) {
    case ACTIONS.MOVE_TASK: {
      ...
      const newData = { ...state.data, columns: newCols };
      return {
        ...state,
        data: newData,
    ...
    case ACTIONS.ADD_TASK: {
      ...
      const newData = { ...state.data, tasks: newTasks, columns: newCols };
      return {
        ...state,
        data: newData,
        history: [...state.history.slice(0, state.historyIndex + 1), newData],
    ...
    case ACTIONS.UPDATE_TASK: {
      const updTask = { ...state.data.tasks[action.payload.id], ...action.payload };
      const newTasks = { ...state.data.tasks, [action.payload.id]: updTask };
    ...
    case ACTIONS.REDO:
      return state.historyIndex < state.history.length - 1 ? { ...state, historyIndex: state.historyIndex + 1 } : state;
...
function getAccentTextColor(accent) {
  return accent === "#B2E4BA" ? "#14532d" : "#fff";
}

function getAccentBadgeTextColor(accent) {
  return accent === "#B2E4BA" ? "#14532d" : accent;
}
    ...
    >
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <span style={{
            background: col.color + "20", color: getAccentBadgeTextColor(col.color),
        ...
        <button
          ...
          onMouseEnter={e => { e.currentTarget.style.background = col.color; e.currentTarget.style.color = getAccentTextColor(col.color); }}
          ...
          {activities.length === 0 ? (
            ...
            activities.slice().reverse().map((act, i) => (
              <div key={i} style={{ padding: "12px", background: "#f8fafc", borderRadius: 10, borderLeft: "3px solid #6366f1" }}>
                ...
                <div style={{ fontSize: 11, color: "#94a3b8", marginTop: 4 }}>{formatUtcTimestamp(act.timestamp)}</div>
              </div>
            ))
          )}
        </div>
...
export default function App() {
  ...
  const currentData = state.history[state.historyIndex];
  const filteredAndSortedData = useMemo(() => {
    const filtered = {
      ...currentData,
      columns: Object.fromEntries(
        Object.entries(currentData.columns).map(([k, col]) => [k, {
          ...col,
          tasks: col.tasks.filter(id => {
            const t = currentData.tasks[id];
            if (!t) return false;
    ...
    if (sortBy === "progress") {
      Object.keys(filtered.columns).forEach(k => {
        filtered.columns[k].tasks.sort((a, b) => {
          const taskA = currentData.tasks[a];
          const taskB = currentData.tasks[b];
    ...
    return filtered;
  }, [search, currentData, sortBy, filterTags]);

  // Calculate stats
  const stats = useMemo(() => {
    const totalTasks = Object.values(currentData.tasks).length;
    const completedTasks = currentData.columns.complete?.tasks.filter(id => currentData.tasks[id]).length || 0;
    const inProgressTasks = currentData.columns.inprogress?.tasks.filter(id => currentData.tasks[id]).length || 0;
    const avgProgress = totalTasks > 0 
      ? Math.round(Object.values(currentData.tasks).reduce((sum, t) => sum + (t.progress / t.total), 0) / totalTasks * 100)
      : 0;
    return { totalTasks, completedTasks, inProgressTasks, avgProgress };
  }, [currentData.tasks, currentData.columns]);
  ...
  const cats = Object.values(currentData.columns);
    ...
    <div style={{ display: "flex", height: "100vh", background: "#f1f5f9", fontFamily: "'DM Sans', 'Inter', sans-serif", overflow: "hidden" }}>
          ...
          <div style={{ padding: "18px 20px 14px", borderBottom: "1px solid #f1f5f9" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 11, color: "#94a3b8" }}>Task.Manager</div>
              </div>
            </div>
          </div>
              ...
              {cats.map(c => (
                <div key={c.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "5px 8px", borderRadius: 8, fontSize: 13, color: "#334155" }}>
                  ...
                  <span style={{ fontSize: 12, color: "#94a3b8", fontWeight: 600 }}>
                    {filteredAndSortedData.columns[c.id]?.tasks.length || 0}
                  </span>
                </div>
              ))}
            </div>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <button onClick={() => setShowActivityLog(!showActivityLog)} style={{ fontSize: 18, cursor: "pointer", color: "#64748b", background: "none", border: "none", padding: 4 }}>📋</button>
        ...
        <div style={{ flex: 1, overflow: "auto", padding: "24px" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 12, color: "#94a3b8", marginTop: 2 }}>
                  {stats.totalTasks} tasks · {stats.completedTasks} completed · {stats.avgProgress}% progress · {stats.inProgressTasks} in progress
                </div>
              </div>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20, gap: 12, flexWrap: "wrap" }}>
            ...
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
              ...
              <select value={sortBy} onChange={e => setSortBy(e.target.value)} style={{
                ...
                padding: "8px 12px", fontSize: 13, fontWeight: 600, color: "#64748b", cursor: "pointer"
              }}>
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
            <button
              ...
              style={{
                background: state.historyIndex === 0 ? "#f1f5f9" : "#fff", 
                border: "1.5px solid #e2e8f0", borderRadius: 8,
                padding: "8px 12px", fontSize: 12, fontWeight: 600, color: state.historyIndex === 0 ? "#cbd5e1" : "#64748b", 
            ...
            <button
              ...
              style={{
                background: state.historyIndex >= state.history.length - 1 ? "#f1f5f9" : "#fff", 
                border: "1.5px solid #e2e8f0", borderRadius: 8,
                padding: "8px 12px", fontSize: 12, fontWeight: 600, color: state.historyIndex >= state.history.length - 1 ? "#cbd5e1" : "#64748b", 
          ...
          <div style={{ display: "flex", gap: 14, alignItems: "flex-start", minHeight: 400 }}>
            {Object.values(filteredAndSortedData.columns).map(col => (
              <Column
                key={col.id}
                col={col}
                tasks={col.tasks.map(id => currentData.tasks[id]).filter(Boolean)}
      ...
      {/* Activity Log */}
      <ActivityLog activities={activities} isOpen={showActivityLog} onClose={() => setShowActivityLog(false)} />

      {/* Toast Notifications */}
      <Toast notifications={state.notifications} onRemove={(id) => dispatch({ type: ACTIONS.REMOVE_NOTIFICATION, payload: id })} />
    </div>
  );
}
